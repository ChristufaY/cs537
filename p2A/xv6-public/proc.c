...
// returns a pointer to lottery winner.
struct proc *hold_lottery(int total_tickets) {
    ...
    int temp_tickets = winner_ticket_number;
    struct proc *p;
    struct proc *winner;
    winner = 0;
    for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
      if(p->state == RUNNABLE) {
        if(p->boostsleft > 0) 
	  temp_tickets = temp_tickets - (2 * p->tickets);
	else 
	  temp_tickets = temp_tickets - p->tickets;
	if(temp_tickets < 0) {
          winner = p;
	  break;
	}
...
scheduler(void)
{
  ...
  for(;;){
    ...
    for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
      if(p->state == RUNNABLE) {
        if(p->boostsleft > 0)
 	  total_tickets = total_tickets + (2 * p->tickets);
	else 
...
wakeup1(void *chan)
{
  ...
  for(p = ptable.proc; p < &ptable.proc[NPROC]; p++)
    if(p->state == SLEEPING && p->chan == chan) {
      if(ticks - p->sleepstart >= p->sleepticks) { 
        p->state = RUNNABLE;
        p->sleepticks = 0;
        p->sleepstart = 0;
      }
