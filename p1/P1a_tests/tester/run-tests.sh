...
print_error_message () {
    ...
    echo "  If a Valgrind test, see memory leaks in tests-out/$testnum.err"
...
check_test () {
    ...
    # valgrind memory leak check only
    isvalgrind=$(grep valgrind $testdir/$testnum.run | wc -l)
    if [ $isvalgrind -gt 0 ]
    then
	    noleak=$(grep "All heap blocks were freed -- no leaks are possible" tests-out/$testnum.err | wc -l)
	    if [ $noleak -gt 0 ]; then
	    	echo 0
	    	return 0
	    else
    ...
    # skip check if expected out is unspecified
    exist=$(test -f $testdir/$testnum.$filetype)
    if (( $? == 1 )); then
            echo 0
            return 0
    fi
